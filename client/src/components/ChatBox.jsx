  ...
  return (
    <div className=' flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
...

      {
        mode === 'image' &&(
          <label  className=' inline-flex items-center gap-2 mb-3 text-sm mx-auto'>
            ...
            <input type="checkbox" name="" className=' cursor-pointer' checked={isPublished} onChange={(e) => setIsPublished(e.target.checked)} id="" />
          </label>
        )
      }
      ...
      <form className=' bg-primary/20 dark:bg-[#583C79]/30 border border-primary dark:border-[#80609F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 items-center' onSubmit={onSubmit}>
        <select onChange={(e)=>setMode(e.target.value)} value={mode} name="" className=' text-sm pl-3 pr-2 outline-none' id="">
